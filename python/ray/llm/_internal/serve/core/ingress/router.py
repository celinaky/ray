...
class LLMRouter:
    ...
    Responses:
        ...
        "request_headers"?: dict}``:
            pick succeeded. ``deployment`` is the Serve deployment name of the
            selected model's server and ``replica_id`` one of its replicas;
            HAProxy's Lua map is keyed ``[deployment][replica_id]``, so both are
            required. ``host`` and ``port`` are informational -- HAProxy
            resolves the endpoint from its own config rather than trusting them.
    ...
    async def route(self, request: Request):
        ...
        # is part of the answer rather than something it re-derives.
        response = {
            "host": host,
            "port": port,
